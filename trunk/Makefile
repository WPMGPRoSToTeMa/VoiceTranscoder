...
#safe optimization
#CFLAGS=$(BASE_CFLAGS) -w -O1 -fpermissive -fno-exceptions

#full optimization
CFLAGS=$(BASE_CFLAGS) -w -O3 -ffast-math -funroll-loops \
...
INCLUDEDIRS=-I$(METAMOD_SRCDIR) -I$(HLSDK_SRCDIR)/common -I$(HLSDK_SRCDIR)/dlls -I$(HLSDK_SRCDIR)/engine -I$(HLSDK_SRCDIR)/pm_shared -Ispeex -Isilk
...
VoiceCodecs/Speex/obj/%.o: VoiceCodecs/Speex/%.c
	$(DO_CC)

VoiceCodecs/SILK/obj/%.o: VoiceCodecs/SILK/%.c
	$(DO_CC)
...
$(DLLNAME).$(SHLIBEXT) : neat $(OBJ)
	$(CC) $(CFLAGS) $(SHLIBLDFLAGS) $(INCLUDEDIRS) $(LDFLAGS) -o $@ $(OBJ)

neat:
	-mkdir obj
	-mkdir VoiceCodecs/Speex/obj
	-mkdir VoiceCodecs/SILK/obj
clean:
	-rm -f $(OBJ)
	-rm -f $(DLLNAME).$(SHLIBEXT)
spotless: clean
	-rm -r obj
	-rm -r VoiceCodecs/Speex/obj
	-rm -r VoiceCodecs/SILK/obj
